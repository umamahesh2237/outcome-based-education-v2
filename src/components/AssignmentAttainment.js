...
const AssignmentAttainment = () => {
  ...
  const [maxMarks, setMaxMarks] = useState({ assignment1: 5, assignment2: 5 });
    ...
    const fetchCourseData = async () => {
      if (filters.courseTitle) {
        try {
          ...
          const initialMappings = ['Assignment-1', 'Assignment-2'].map(assignment => ({
            ...
            const fetchExistingData = async () => {
              try {
                const [mappingRes, marksRes] = await Promise.all([
                  axios.get(`${API_BASE_URL}/api/assignment-attainment/fetch`, {
                    params: { ...filters, batch, academicYear }
                  }),
                  axios.get(`${API_BASE_URL}/api/assignment-attainment/marks/fetch`, {
                    params: { ...filters, batch, academicYear }
                  })
                ]);
                ...
                if (mappingRes.data) {
                  ...
                  setMaxMarks({
                    assignment1: mappingRes.data.mappings[0]?.maxMarks || 5,
                    assignment2: mappingRes.data.mappings[1]?.maxMarks || 5
                  });
                }
  ...
  const handleMappingChange = (assignmentIndex, coIndex, value) => {
    setMappings(prevMappings => {
      const newMappings = [...prevMappings];
      newMappings[assignmentIndex].coMappings[coIndex].value = value;
      return newMappings;
    });
  };
  ...
  const handleMarkChange = (index, field, value) => {
    ...
    if (field.startsWith('assignment') && numValue !== '') {
      const assignmentNum = field === 'assignment1' ? 1 : 2;
      if (numValue > maxMarks[`assignment${assignmentNum}`]) {
        alert(`Value cannot exceed maximum marks (${maxMarks[`assignment${assignmentNum}`]})`);
        return;
      }
    }
  ...
  const saveMapping = async () => {
    ...
    try {
      await axios.post(`${API_BASE_URL}/api/assignment-attainment/save`, {
        ...filters,
        batch,
        academicYear,
        mappings
      });
      alert('Assignment mapping saved successfully!');
      setError(null);
    } catch (err) {
  ...
  const saveMarks = async () => {
    ...
    try {
      ...
      await axios.post(`${API_BASE_URL}/api/assignment-attainment/marks/save`, {
        ...filters,
        batch,
        academicYear,
        marks: validMarks
  ...
  const calculateAttainments = () => {
    ...
    const attainments = ['assignment1', 'assignment2'].map(assignment => {
      const maxMark = maxMarks[assignment];
      const target = maxMark * 0.6;
      const studentsReachingTarget = validMarks.filter(m => m[assignment] >= target).length;
    ...
    <div>
      ...
      <div className="table-responsive mt-4">
        <table className="table table-bordered">
          ...
          <tbody>
            {mappings.map((mapping, assignmentIndex) => (
              <tr key={mapping.assignmentNumber}>
                <td>{mapping.assignmentNumber}</td>
                <td>{mapping.maxMarks}</td>
                {mapping.coMappings.map((coMapping, coIndex) => (
                  <td key={coMapping.coNumber}>
                    <select
                      value={coMapping.value}
                      onChange={(e) => handleMappingChange(assignmentIndex, coIndex, e.target.value)}
        ...
        <table className="table table-bordered">
          <thead>
            ...
            <tr>
              ...
              <td>{maxMarks.assignment1}</td>
              <td>{maxMarks.assignment2}</td>
            </tr>
          </thead>
          <tbody>
            {marks.map((row, index) => (
              <tr key={index}>
                ...
                <td>
                  <input
                    ...
                    onChange={(e) => handleMarkChange(index, 'assignment1', e.target.value)}
                    className="form-control form-control-sm"
                    min="0"
                    max={maxMarks.assignment1}
                  />
                </td>
                <td>
                  <input
                    ...
                    onChange={(e) => handleMarkChange(index, 'assignment2', e.target.value)}
                    className="form-control form-control-sm"
                    min="0"
                    max={maxMarks.assignment2}
                  />
                </td>
              </tr>
            ))}
  ...
  return (
    <div className="regulation-form mt-4">
      ...
      <div className="d-flex gap-2 mb-4 mt-3">
        ...
        >
          <b>Print Assignment Attainments</b>
        </button>
      </div>
...
export default AssignmentAttainment;